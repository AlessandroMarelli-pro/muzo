...
} from 'src/clean-arch/application/use-cases/third-party-sync';
...
export class ThirdPartySyncResolver {
  ...
  async authenticateTidal(
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to authenticate with TIDAL';
      return { success: false, message };
    }
  }
  ...
  async authenticateSpotify(
    ...
    try {
      await this.exchangeSpotifyCodeUseCase.execute(
        code,
        codeVerifier,
        userId,
      );
