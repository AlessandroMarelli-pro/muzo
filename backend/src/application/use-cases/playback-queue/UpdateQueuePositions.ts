...
export class UpdateQueuePositionsUseCase {
  ...
  async execute(positions: UpdateQueuePositionInput[]): Promise<QueueItemWithTrack[]> {
    ...
    if (missingCount > 0) {
      const missingTrackIds = trackIds.filter((_, i) => existingItems[i] === null);
      throw createNotFoundError(`Tracks not found in queue: ${missingTrackIds.join(', ')}`);
    }
