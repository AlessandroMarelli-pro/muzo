...
export class PlaylistStatsQuery implements IPlaylistStatsQuery {
    ...
    return this.prisma.$queryRaw<RawPlaylistStatsRow[]>`
        ...
        genre_aggregated AS (
          ...
          SELECT
            ...
            STRING_AGG(DISTINCT gs.genre_name, ',') as all_genres
          FROM genre_stats gs
        ...
        subgenre_aggregated AS (
          ...
          SELECT
            ...
            STRING_AGG(DISTINCT ss.subgenre_name, ',') as all_subgenres
  ...
  async getPlaylistsStats(): Promise<PlaylistStatsDto[]> {
    const currentUserId = getCurrentUserId();
    return this.prisma.$queryRaw<RawPlaylistStatsRow[]>`
        WITH track_stats AS (
          -- First CTE: Get unique track data without duplicates
          ...
          FROM playlist_tracks pt
          JOIN music_tracks t ON pt."trackId" = t.id
          WHERE pt."createdById" = ${currentUserId}
        ),
        genre_stats AS (
          -- Get genres for tracks
          SELECT DISTINCT
        ...
        genre_aggregated AS (
          ...
          SELECT
            ...
            gs."createdById"
          FROM genre_stats gs
          GROUP BY gs."playlistId", gs."createdById"
        ),
        subgenre_aggregated AS (
          -- Aggregate subgenre data separately
          SELECT
        ...
        FROM playlists p
        LEFT JOIN final_stats fs ON p.id = fs."playlistId"
        WHERE p."createdById" = ${currentUserId}
