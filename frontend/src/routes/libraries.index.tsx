...
function LibrariesPage() {
  ...
  const scanIncompleteTracksMutation = useScanIncompleteTracks();
  ...
  const handleScanIncompleteTracks = (libraryId: string) => {
    scanIncompleteTracksMutation.mutate(
      { libraryId },
      {
        onSuccess: (sessionId) => {
          if (sessionId) {
        ...
        onError: (error) => {
          console.error('Failed to scan incomplete tracks:', error);
        },
      },
    );
  };
