...
const addTracksToQueue = async (trackIds: string[]): Promise<QueueItem[]> => {
	const data = await graffleClient.request<{ addTracksToQueue: QueueItem[] }>(
		ADD_TRACKS_TO_QUEUE,
		{ trackIds }
	);
...
const resetQueue = async (): Promise<boolean> => {
	const data = await graffleClient.request<{ resetQueue: boolean }>(
		RESET_QUEUE
	);
...
const addTrackToQueueSuccessToast = (
	data: QueueItem,
