...
export function CreatePlaylistDialog({
	...
	const [subgenreSelectionMode, setSubgenreSelectionMode] = useState<
		'exact' | 'contain'
	>('exact');
		...
		try {
					...
					? {
							...
							atmospheres:
								selectedAtmospheres.length > 0
									...
									: undefined,
							libraryIds:
								selectedLibraries.length > 0 ? selectedLibraries : undefined,
							tempo:
			...
			await createPlaylist({
				...
				subgenreSelectionMode:
					selectedSubgenres.length > 0 ? subgenreSelectionMode : undefined,
			} as any);
					...
					<div className="grid gap-4 py-4 ">
						...
						<Collapsible className="data-[state=open]:bg-muted rounded-md ">
							...
							<CollapsibleContent className="flex flex-col items-start gap-2 p-2.5  text-sm w-full">
								<Item size="sm" variant="outline">
									...
									<ItemContent className="w-full">
										<ItemDescription className="w-full line-clamp-none text-wrap">
											Tracks will be automatically selected based on the filters
											you select.
										</ItemDescription>
									</ItemContent>
								</Item>
								...
								{/* Atmospheres Filter */}
								<Field orientation="horizontal">
									<FieldLabel htmlFor="atmospheres-filter">
										Atmospheres
									</FieldLabel>
									<MultiSelect
