...
import { capitalizeEveryWord, cn, formatSimilarity, formatTime, toCamelotCode } from '@/lib/utils';
import { useNavigate } from '@tanstack/react-router';
import { AudioLines, Brain, ListMusic, Pause, Play, Plus } from 'lucide-react';
...
export const TrackRecommendationsCardSkeleton = ({ index }: { index: number }) => {
  return (
    <div key={`skeleton-recommendations-card-${index}`} className={cn(REC_GRID, 'py-2.5')}>
...
export const TrackRecommendationsCard = ({
  ...
  const title = track.title ? capitalizeEveryWord(track.title) : 'Unknown track';
