...
export function SwipeTrack({ track, onLike, onDislike, onBanger }: SwipeTrackProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setAudioBars([Math.random() * 40 + 20, Math.random() * 50 + 40, Math.random() * 40 + 30]);
    }, 150);
        ...
        <div className="flex flex-col gap-2">
          <div className="flex flex-row gap-2">
            {track?.genres?.map((genre) => (
              <Badge variant="outline" className="text-xs capitalize border-none">
                {genre}
              </Badge>
            ))}
          </div>
          <div className="flex flex-row gap-2">
            {track?.subgenres?.map((subgenre) => (
              <Badge variant="secondary" className="text-xs capitalize border-none">
                #{subgenre}
              </Badge>
            ))}
          </div>
...
export function SwipeView({ track, isLoading, onLike, onDislike, onBanger }: SwipeViewProps) {
  if (isLoading) {
    return (
  ...
  return <SwipeTrack track={track} onLike={onLike} onDislike={onDislike} onBanger={onBanger} />;
}
