...
export const LibraryCard: React.FC<LibraryCardProps> = ({
  ...
  const handleForceScan = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
    e.preventDefault();
  ...
  return (
    <Card
      className="hover:shadow-lg  cursor-pointer hover:scale-103 transition-all min-w-sm"
      ...
      <CardContent className="space-y-4">
        ...
        <div className="text-sm text-muted-foreground">
          ...
          {library.lastIncrementalScanAt && (
            <div>Last Incremental: {formatDate(library.lastIncrementalScanAt)}</div>
          )}
        </div>
        ...
        <div className="flex space-x-2 pt-2 w-full">
          ...
          <Button
            variant="outline"
            size="iconSm"
            title="Force scan (re-analyze every track)"
            aria-label="Force scan"
