...
- **`backend/src/clean-arch/infrastructure/`** — Clean-arch-specific implementations of application **ports** used only by clean-arch use cases/adapters.
...
**Recommendation:** Prefer one source of truth per concern. For Prisma and image reading, either use only root `backend/src/infrastructure/` or only `clean-arch/infrastructure/` and update all adapters to use that single location. Currently both exist and some repositories use root Prisma while others use clean-arch Prisma (see below).

## Clean-arch infrastructure (`backend/src/clean-arch/infrastructure/`)
...
## Elasticsearch (`clean-arch/infrastructure/external-services/elasticsearch/`)
...
Persistence adapters may import Prisma from **clean-arch** (`clean-arch/infrastructure/database/prisma.service.ts`) via relative paths from `adapters/persistence/` (e.g. `../../infrastructure/database/prisma.service`). Image reader and waveform generator are in **clean-arch** infrastructure and wired in `AdaptersPersistenceModule`.

## Port bindings (summary)

| Port                                                 | Implementation                   | Location                                      |
| ---------------------------------------------------- | -------------------------------- | --------------------------------------------- |
| `AUDIO_WAVEFORM_GENERATOR`                           | `WaveformGenerator`              | `clean-arch/infrastructure/audio/`             |
| `IMAGE_FILE_READER`                                  | `FileSystemImageReader`          | `clean-arch/infrastructure/filesystem/`        |
| `TRACK_INDEXER_PORT`                                 | `ElasticsearchTrackIndexerAdapter` | `clean-arch/infrastructure/.../elasticsearch/` |
| `RECOMMENDATION_SEARCH_PORT`                         | `RecommendationSearchAdapter`    | `clean-arch/infrastructure/.../elasticsearch/` |
| `PrismaService` (injected into repositories/queries) | `PrismaService`                  | `clean-arch/infrastructure/database/` (or root) |
