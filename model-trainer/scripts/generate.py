"""
Create Personal Music Dataset

Extract features from personal music files organized by genre folders
and create a properly labeled dataset for training.

Optimized for high-performance processing with multiprocessing and GPU acceleration.
"""
...
def extract_features_worker(args: Tuple[str, str, str, str]) -> Dict:
    """
    ...
    try:
        ...
        for i in range(1, 21):  # mfcc1 to mfcc20
            if i <= len(mfcc_array):
                ...
                gtzan_features[f"mfcc{i}_var"] = 0.0  # We don't have variance, use 0
            else:
...
def process_files_parallel(
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_args = {
            executor.submit(extract_features_worker, args): args for args in file_args
        }
...
def create_personal_dataset(
    ...
    """
    Create a labeled dataset from personal music files organized by genre/subgenre.

    This function supports incremental processing - if the output CSV file already exists,
    it will load the existing dataset and skip processing files that are already included.
    This is useful for growing the dataset incrementally without reprocessing existing files.

    Optimized for high-performance processing with multiprocessing support.
    ...
    """
    print("=" * 60)
    print("CREATING PERSONAL MUSIC DATASET")
    print("=" * 60)
    ...
    for genre_folder in genre_folders:
        ...
        if subgenre_folders:
            ...
            for subgenre_folder in subgenre_folders:
                ...
                if not music_files:
                    print(f"     ⚠️  No music files found in {subgenre_name}")
                    continue
                ...
                for file_path in music_files:
                    ...
                    try:
                        ...
                        if features is not None:
                            ...
                            # Chroma features (convert from array to mean/var)
                            chroma_array = np.array(features["chroma"])
                            gtzan_features["chroma_stft_mean"] = float(
                                np.mean(chroma_array)
                            )
                            gtzan_features["chroma_stft_var"] = float(
                                np.var(chroma_array)
                            )

                            # RMS features (convert from energy)
                            gtzan_features["rms_mean"] = features["energy"]
                            gtzan_features["rms_var"] = (
                                0.0  # We don't have variance, use 0
                            )

                            # Spectral features
                            gtzan_features["spectral_centroid_mean"] = features[
                                "spectral_centroid"
                            ]
                            ...
                            # Rolloff
                            gtzan_features["rolloff_mean"] = features[
                                "spectral_rolloff"
                            ]
                            gtzan_features["rolloff_var"] = 0.0

                            # Zero crossing rate
                            gtzan_features["zero_crossing_rate_mean"] = features[
                                "zero_crossing_rate"
                            ]
                            gtzan_features["zero_crossing_rate_var"] = 0.0
                            ...
                            for i in range(1, 21):  # mfcc1 to mfcc20
                                if i <= len(mfcc_array):
                                    gtzan_features[f"mfcc{i}_mean"] = float(
                                        mfcc_array[i - 1]
                                    )
                            ...
                            gtzan_features["label"] = subgenre_name

                            # Additional columns
                            gtzan_features["genre"] = genre_name
                            gtzan_features["subgenre"] = subgenre_name
                            gtzan_features["dataset"] = "personal"
                            gtzan_features["extraction_date"] = pd.Timestamp.now()

                            subgenre_features.append(gtzan_features)
                            successful_extractions += 1

                            if successful_extractions % 5 == 0:
                                print(
                                    f"     Processed {successful_extractions}/{len(music_files)} files..."
                                )
                ...
                print(
                    f"     ✅ Successfully processed {successful_extractions}/{len(music_files)} files"
                )
                if skipped_files > 0:
                    print(f"     ⏭️  Skipped {skipped_files} already processed files")
                all_features.extend(subgenre_features)
            ...
            if not music_files:
                print(f"   ⚠️  No music files found in {genre_name}")
                continue
            ...
            for file_path in music_files:
                ...
                try:
                    ...
                    if features is not None:
                        ...
                        # Label (use genre name when no subgenres)
                        gtzan_features["label"] = genre_name

                        # Additional columns
                        gtzan_features["genre"] = genre_name
                        gtzan_features["subgenre"] = ""
                        gtzan_features["dataset"] = "personal"
                        gtzan_features["extraction_date"] = pd.Timestamp.now()

                        genre_features.append(gtzan_features)
                        successful_extractions += 1
    ...
    if not all_features and existing_df is None:
        print("❌ No features extracted from any files and no existing dataset found")
        return False
    elif not all_features and existing_df is not None:
        print("ℹ️  No new features extracted, but existing dataset will be preserved")
        df = existing_df
    else:
    ...
    print("\n📊 Dataset Summary:")
    if existing_df is not None and all_features:
    ...
    with open(summary_file, "w") as f:
        f.write("Personal Music Dataset Summary\n")
        f.write("=" * 40 + "\n")
        if existing_df is not None and all_features:
...
def main():
    """Main function to create personal dataset."""
    import argparse

    parser = argparse.ArgumentParser(description="Create personal music dataset")
    parser.add_argument(
        "--input",
        "-i",
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="../data/personal_features.csv",
    ...
    success = create_personal_dataset(args.input, args.output, args.workers)
