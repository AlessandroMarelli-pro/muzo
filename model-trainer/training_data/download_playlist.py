...
class VPNManager:
            ...
            try:
                ...
                if self.nordvpn_settings is None:
                    ...
                    try:
                        ...
                        self.nordvpn_settings = initialize_VPN(
                            save=1, area_input=["random countries europe 10"]
                        )
                ...
                print_error(
                    "nordvpn-switcher package not installed. Install with: pip install nordvpn-switcher"
                )
                return False
        except Exception as e:
    ...
    def _manual_nordvpn_connect(self, location):
        ...
        try:
            ...
            if result.returncode == 0:
                self.current_location = location
                print_success(
                    f"Successfully connected to NordVPN server in {location} (manual)"
                )
                time.sleep(3)
                return True
            else:
            ...
            print_error(
                "NordVPN CLI not found. Please ensure NordVPN is properly installed."
            )
            return False
    ...
    def _manual_nordvpn_disconnect(self):
        ...
        try:
            result = subprocess.run(
                ["nordvpn", "disconnect"], capture_output=True, text=True, timeout=30
            )
            if result.returncode == 0:
    ...
    def change_vpn_location(self):
        ...
        available_locations = [
            loc for loc in self.vpn_locations if loc != self.current_location
        ]
...
def count_existing_audio_files(directory):
    ...
    try:
        ...
        audio_files = [
            f
            for f in files
            if f.endswith((".flac", ".mp3", ".m4a", ".webm", ".wav", ".ogg", ".opus"))
        ]
...
def download_playlist(
    ...
    if existing_files >= max_files:
        print_warning(
            f"Directory already contains {existing_files} audio files (>= {max_files})"
        )
        ...
        try:
            ...
            if stderr_output and check_bot_detection_error(stderr_output):
                ...
                if vpn_manager and vpn_manager.is_vpn_available():
                    ...
                    print_error(
                        "YouTube is asking you to sign in to confirm you're not a bot."
                    )
                    ...
                    print_info(
                        "3. Try again later when the bot detection is less active"
                    )
            ...
            if return_code == 0:
                ...
                try:
                    ...
                    audio_files = [
                        ...
                        for f in files
                        if f.endswith(
                            (".flac", ".mp3", ".m4a", ".webm", ".wav", ".ogg", ".opus")
                        )
                    ]
