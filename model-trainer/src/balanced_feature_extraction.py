...
class BalancedSegmentationExtractor:
    ...
    def analyze_dataset(self, dataset_path: Union[str, Path]) -> Dict:
        ...
        logger.info(
            f"  Total estimated duration: {analysis['total_duration'] / 3600:.1f} hours"
        )
        ...
        for genre_name, genre_data in self.dataset_analysis["genres"].items():
            ...
            for subgenre_name, subgenre_data in subgenres.items():
                ...
                if current_samples > 0:
                    ...
                    chunks_needed_per_file = max(
                        1, int(np.ceil(chunks_needed_per_file))
                    )
                else:
                ...
                if available_duration > 0 and current_samples > 0:
                    natural_chunks_per_file = (
                        available_duration / current_samples / chunk_duration
                    )
                    if natural_chunks_per_file < chunks_needed_per_file:
                        # Need overlap to achieve target
                        overlap_ratio = 1 - (
                            natural_chunks_per_file / chunks_needed_per_file
                        )
                ...
                logger.info(
                    f"    {subgenre_name}: {current_samples} → {target_per_subgenre} samples"
                )
    ...
    def extract_features_from_audio_segment(
        ...
        try:
            ...
            logger.warning(
                "soundfile not available, falling back to manual feature extraction"
            )
    ...
    def _extract_features_manual(self, audio_segment: np.ndarray, sr: int) -> Dict:
        ...
        try:
            ...
            features["zero_crossing_rate"] = (
                self.feature_extractor.extract_zero_crossing_rate(audio_segment)
            )
            ...
            features["chroma"] = self.feature_extractor.extract_chroma_features(
                audio_segment
            ).tolist()
            ...
            valence, danceability = self.feature_extractor.extract_valence_danceability(
                audio_segment
            )
    ...
    def create_balanced_features(
        ...
        if not AUDIO_PROCESSING_AVAILABLE:
            raise ValueError(
                "Audio processing (librosa) is required for feature extraction"
            )
        ...
        if show_progress and PROGRESS_BAR_AVAILABLE:
            pbar = tqdm(
                ...
                desc="Creating balanced features",
                unit="samples",
            )
        else:
            pbar = None
        ...
        for genre_name, genre_plan in self.segmentation_plan.items():
            ...
            for subgenre_name, subgenre_plan in genre_plan.items():
                ...
                for audio_file in files:
                    ...
                    try:
                        ...
                        if duration < self.min_segment_duration:
                            logger.warning(
                                f"File {audio_file.name} too short ({duration:.1f}s), skipping"
                            )
                            continue
                        ...
                        while (
                            start + chunk_samples <= len(y)
                            and file_chunks < chunks_needed_per_file
                            and subgenre_samples < target_samples
                        ):
                            # Extract chunk
                            chunk = y[start : start + chunk_samples]

                            # Extract features using the proven extract_all_features method
                            features = self.extract_features_from_audio_segment(
                                chunk, sr
                            )
                ...
                logger.info(
                    f"    Created {subgenre_samples} samples for {subgenre_name}"
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create balanced feature dataset with dynamic chunking"
    )
    parser.add_argument("--dataset", required=True, help="Path to dataset directory")
    parser.add_argument(
        "--output", default="data/features_balanced.csv", help="Output CSV path"
    )
    parser.add_argument(
        "--target-per-genre", type=int, default=1000, help="Target samples per genre"
    )
    parser.add_argument(
        "--min-chunk-duration",
        type=float,
    ...
    parser.add_argument(
        "--n-mfcc", type=int, default=13, help="Number of MFCC coefficients to extract"
    )
    parser.add_argument(
        "--n-chroma", type=int, default=12, help="Number of chroma bins"
    )
